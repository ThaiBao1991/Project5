...
import psutil #Check file isOpen
import time
...
Layout = [    
        ...
        [sg.Button('Update Form',key="Start", size=(10, 2), button_color=('blue', 'grey'), font=('Helvetica', 10,'bold'), pad=(10, 10),enable_events=True),
            sg.Button('Open Form',key="Open", size=(10,2),button_color=('yellow','grey'),enable_events=True,font=('Helvetica',10,'bold'),pad=(10, 10)),
            sg.Button('Transfer Data',key="CopyData", size=(10,2),button_color=('lightgreen','grey'),enable_events=True,font=('Helvetica',10,'bold'),pad=(10, 10))],
...
# Tạo của sổ TransferData
def create_transferdata_window():
    layout = [
        [sg.Text("Đường dẫn đến file access cần đưa dữ liệu vào :",font=('Helvetica', 11,'bold')),sg.InputText(disabled=True,size=(50, 1), key='LinkData'),sg.FilesBrowse(key='BtnOpenLinkData')],
        [sg.VPush()],
        [sg.Push(),sg.Button('OK', size=(10, 2), button_color=('lightgreen', 'grey'), font=('Helvetica', 8, 'bold'), pad=(0, 10), enable_events=True), sg.Button('Cancel', size=(10, 2), button_color=('red', 'grey'), font=('Helvetica', 8, 'bold'), pad=(10, 0), enable_events=True)],
        [sg.VPush()],
        #  [ sg.Button('OK', size=(10, 2), button_color=('lightgreen', 'grey'), font=('Helvetica', 8, 'bold'), pad=(0, 0), enable_events=True), sg.Push(), sg.Button('Cancel', size=(10, 2), button_color=('red', 'grey'), font=('Helvetica', 8, 'bold'), pad=(0, 0), enable_events=True)]
               ]
    
    return sg.Window('Đường dẫn đến file lưu trữ', layout, return_keyboard_events=True, margins=(10, 10))

# window=sg.Window('Chương trình lấy dữ liệu từ máy OGP vào biểu ghi chép',Layout,size=(650,450),resizable=False,finalize=True)
window=sg.Window('Chương trình lấy dữ liệu từ máy OGP vào biểu ghi chép',Layout,resizable=False,finalize=True,return_keyboard_events=True)
window.bind('<Alt-s>', 'alt_s')
...
while True:
    ...
    if event == sg.WIN_CLOSED:
        ...
        # print("Chạy ALT + S") #Tham khảo print Alt + S
        ...
        while True:
            ...
            if event in (sg.WINDOW_CLOSED, 'Cancel'):
                ...
                linkfilexcel = values['LinkData']
                if not linkfilexcel:
                    sg.popup("Vui lòng chọn đường link file excel cần chuyển dữ liệu")
                else:
                    file_extension = os.path.splitext(linkfilexcel)[1]
                    if(file_extension !=".xlsx" and file_extension !=".xlsb"):
                        sg.popup("File chọn phải là một file excel")
                    else :  
                        # if(file_save_path):
                        if(True):
                            # source_wb= load_workbook(file_save_path )
                            # destination_wb= load_workbook(linkfilexcel)
                            source_wb= load_workbook('C:\\Users\\12953 bao\\Desktop\\47598F-505461U-1.xlsx')
                            destination_wb= load_workbook('C:/Users/12953 bao/Desktop/47598F-505461U-2.xlsx')
                            source_ws = source_wb.active
                            dest_ws = destination_wb.active
                            for row in range(9,19) :
                                source_cell = source_ws[f'O{row}']
                                dest_cell = dest_ws[f'O{row}']
                                dest_cell.value = source_cell.value
                            for row in range(9,19) :
                                source_cell = source_ws[f'T{row}']
                                dest_cell = dest_ws[f'T{row}']
                                dest_cell.value = source_cell.value    
                            for row in range(9,19) :
                                source_cell = source_ws[f'AD{row}']
                                dest_cell = dest_ws[f'AD{row}']
                                dest_cell.value = source_cell.value    
                            ...
                            for row in range(39,48) :
                                ...
                                dest_cell.value = source_cell.value    
                            for row in range(39,48) :
                                source_cell = source_ws[f'Y{row}']
                                dest_cell = dest_ws[f'Y{row}']
                                dest_cell.value = source_cell.value    
                            for row in range(39,48) :
                                source_cell = source_ws[f'AD{row}']
                                dest_cell = dest_ws[f'AD{row}']
                            ...
                            for row in range(86,95) :
                                source_cell = source_ws[f'P{row}']
                                dest_cell = dest_ws[f'P{row}']
                                dest_cell.value = source_cell.value    
                            for row in range(86,95) :
                                source_cell = source_ws[f'T{row}']
                                dest_cell = dest_ws[f'T{row}']
                            ...
                            for row in range(101,110) :
                                source_cell = source_ws[f'C{row}']
                                dest_cell = dest_ws[f'C{row}']
                                dest_cell.value = source_cell.value   
                            for row in range(101,110) :
                                source_cell = source_ws[f'F{row}']
                                dest_cell = dest_ws[f'F{row}']
                                dest_cell.value = source_cell.value  
                            for row in range(101,110) :
                                source_cell = source_ws[f'I{row}']
                                dest_cell = dest_ws[f'I{row}']
                                dest_cell.value = source_cell.value 
                            for row in range(101,110) :
                                source_cell = source_ws[f'O{row}']
                                dest_cell = dest_ws[f'O{row}']
                            ...
                            destination_wb.save(linkfilexcel)
                            sg.popup("Đã hoàn tất copy dữ liệu, bạn hãy kiểm tra lại nhé")
                            break
    elif event == 'Close':
        break
...
# pyinstaller --onefile --noconsole Example5-ver1.py
# pyinstaller --onefile --noconsole --add-data 'data_form_config.txt;.' Example5-ver1.py
# pyinstaller --onefile --noconsole --add-data 'data_form_config.txt:.' Example5-ver1.py
# & "C:\Users\12953 bao\AppData\Roaming\Python\Python312\Scripts\pyinstaller" --onefile --noconsole --add-data 'data_form_config.txt;.' "C:\Users\12953 bao\Desktop\desktop\work\Project\Python\BasicLearnPython\W3schools\Python Tutorial\ProjectInMBC\OJBTransferDataToExcel\Example5-ver1.py"

# & "C:\Users\12953 bao\AppData\Roaming\Python\Python312\Scripts\pyinstaller" --onefile --noconsole --add-data "C:\Users\12953 bao\Desktop\desktop\work\Project\Python\BasicLearnPython\W3schools\data_form_config.txt;." "C:\Users\12953 bao\Desktop\desktop\work\Project\Python\BasicLearnPython\W3schools\Python Tutorial\ProjectInMBC\OJBTransferDataToExcel\Example5-ver1.py"

# & "C:\Users\12953 bao\AppData\Roaming\Python\Python312\Scripts\pyinstaller" --onefile --noconsole --add-data "C:\Users\12953 bao\Desktop\desktop\work\Project\Python\BasicLearnPython\W3schools\data_form_config.txt;." "C:\Users\12953 bao\Desktop\desktop\work\Project\Python\BasicLearnPython\W3schools\Python Tutorial\ProjectInMBC\Measurement equip connect\MECP\MECP.py"
