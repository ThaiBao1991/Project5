                ...
                try:
                    ...
                    # Xử lý file .xls
                        ...
                        if khach_hang == "CANON":
                            ...
                            print("dong xoa la", rows_to_delete)
                            print("tong chieu dai la :",total_deleted_height)  
                            ...
                            _move_all_objects_up(ws, total_deleted_height/2)
                            ...
                            # Xét các dòng W7 -> W32 (của toàn sheet)
                            rows_to_delete = []
                            for j in range(13, 32):  # W7 đến W32 (bao gồm cả 31)
                                cell = ws.range(f"AE{j}")
                                cell_W = ws.range(f"AE{j}").value
                                cell_formula = cell.formula
                                if isinstance(cell_W, str) and cell_W.startswith("#"):
                                    rows_to_delete.append(j)
                                elif cell_W is None:
                            ...
                            # Xóa các dòng W7-W31 theo range R-BR, xóa từ dòng lớn đến nhỏ
                            for j in sorted(rows_to_delete, reverse=True):
                                ws.range(f"R{j}:AZ{j}").delete(shift="up")
                            ...
                            order_no_text = "ORDER No:"
                            if order_no_list:
                                ws.range("X3").value = f"{order_no_text} {', '.join(order_no_list)}"
                            wb.save()
                            wb.close()
                            app.quit()
                            if found_lot:
